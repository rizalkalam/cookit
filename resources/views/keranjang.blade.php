    ...
    <div class="section-bskt">
            ...
            <div class="list-bskt">
                ...
                <div class="item" data-id-cart="{{ $item->id }}">
                    ...
                    <div class="right-list-bskt">
                        <div class="price-con-bskt">Rp.<span class="unit-price-bskt" data-unit-price="{{ $item->menu->price }}">{{ $item->menu->price }}</span></div>
        ...
        <div class="sec-total-bskt">
            ...
            <div class="left-total-bskt">
                <p>Total (<span class="total-items">{{ $cart_items->sum('qty') }}</span> Product)</p>
                <div class="price-con-total-bskt">Rp.<span class="total-price">{{ $cart_items->sum(function($item) { return $item->menu->price * $item->qty; }) }}</span></div>
                <a href="/check_out">
...
<input type="hidden" name="shipping_cart" value="{{ $shippingCost = auth()->user()->addres->district->shipping_cost }}">
<input type="hidden" name="total_price" value="{{ $subtotal = $cart_items->sum(function($item) { return $item->menu->price * $item->qty; }) }}">
...
<div id="deleteModal" style="display: none;">
    <div id="editTosentModal" class="reveal-modal-tutorial">
        ...
        <form action="/cart/delete/{{ $item->id }}" method="POST" id="deleteForm">
            @csrf
            @method('DELETE')
